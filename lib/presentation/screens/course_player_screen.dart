...
class _CoursePlayerBodyState extends State<_CoursePlayerBody>
  ...
  void initState() {
    ...
    if (widget.existingController != null) {
      // Returning from mini player — resume existing controller
  ...
  void _minimize() {
    ...
    if (_vpc != null) {
      ...
      // Pause BEFORE disposing Chewie — stops the decoder from flooding the
      // ImageReader buffer while the old surface is torn down.
      vpcToTransfer.pause();
      _chewie?.dispose();
      _chewie = null;
      _vpc = null; // prevent dispose() from touching it
        ...
        children: [
          Text(
            ...
            style: const TextStyle(
                fontSize: 11, color: Colors.white54, fontWeight: FontWeight.normal),
              ...
              .map((q) => PopupMenuItem<String>(
                    ...
                    child: Row(children: [
                      Icon(
                        ...
                        size: 16,
                        color: _quality == q ? _kAccent : Colors.white54,
                      ),
                      const SizedBox(width: 8),
                      Text(q,
                          style: TextStyle(
                              color: _quality == q ? Colors.white : Colors.white70,
                              fontSize: 14)),
                    ]),
                  ))
              .toList(),
  ...
  Widget _videoBg() => Container(
        ...
        child: const Center(
            child:
                Icon(Icons.play_lesson_rounded, color: Colors.white10, size: 72)),
      );
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Lesson ${_currentIndex + 1} of $total',
                  style: const TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                ...
                Text(
                  '$doneCount/$total lessons complete · ${(pct * 100).round()}%',
          ...
          if (!_isCurrentDone) const SizedBox(width: 8),
          // Next
          _navBtn(
    ...
    return ListView(
      ...
      children: [
        ...
        ...sectionOrder.asMap().entries.map((entry) {
          ...
          final doneCount = indices
              .where((i) => _completed
                  .contains(widget.lessons[i]['id']?.toString() ?? ''))
              .length;
          final sectionPct =
              indices.isEmpty ? 0.0 : doneCount / indices.length;
          ...
          return Column(
            ...
            children: [
              ...
              // Lesson tiles
              if (isOpen)
                ...indices.map((li) => _buildLessonTile(li)),
  ...
  Widget _buildLessonTile(int lessonIdx) {
    ...
    final isProcessing = hasVid && (status == 'PROCESSING' || status == 'UPLOADED');
    final playable =
  ...
  Widget _buildOverviewTab() {
    ...
    final nextLesson = _hasNext ? widget.lessons[_currentIndex + 1] : null;
  ...
  Widget _buildUpNextCard(Map<String, dynamic> next) {
    ...
    final playable = (hasVid && (status == 'READY' || status == 'APPROVED')) ||
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(
                playable ? Icons.play_arrow_rounded : Icons.lock_outline_rounded,
                color: playable ? _kNavy : Colors.grey,
                size: 22,
              ),
            ),
